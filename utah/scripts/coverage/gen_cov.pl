...
my $LCOV_HOME = "$COMPILER_SOURCES_HOME/lcov/bin";
...
my $NUMBER_OF_TESTCASES = 10;

my $CSMITH_OPTS = " --no-paranoid --bitfields  --packed-struct ";

my $CSMITH_TIMEOUT = 60;

my $COMPILER_TIMEOUT = 600;

my $MIN_PROGRAM_SIZE = 1000;
...
sub test_compilers($) {
    ...
    foreach my $compiler (keys %compilers) {
        ...
        foreach my $opt (@$opts) {
            ...
            my $command = "./RunSafely.sh $COMPILER_TIMEOUT 1 /dev/null $compilerout $compiler_exec $opt $extra_opts $cfile -o $exe -I/home/extended_csmith/coverage/csmith/runtime";
...
sub do_one_test() {
    ...
    system "rm -rf ./*";
    my $cmd = "$COV_BASE_HOME/csmith/src/csmith $CSMITH_OPTS --output $cfile";
	system "cp ../../csmith/utah/scripts/jxyang/RunSafely.sh .";
	system "chmod +x RunSafely.sh";
